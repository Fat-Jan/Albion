...
KOOK ←─WebSocket─→ [机器人进程 (Python/khl.py)]
                        │
                        ├─ albion/ 数据层 ──HTTP──→ gameinfo-ams / AODP(europe) / albionbb(eu)
                        │     (自带缓存 + 限流 + 退避)
                        │
...
要点（来自接口文档实测）：
- 区服一致：战斗 `gameinfo-ams`、市场 `europe.albion-online-data.com`、ZvZ `api.albionbb.com/eu`。
- KOOK 频道作用域一致：欧服实例使用 `KOOK_REGION_CODE=eu`，只响应和发送到 `eu-` 前缀频道；无前缀或其他区服前缀频道静默。
- 先 `/search` 拿 ID 再查详情。
- AODP 限流 **180/分**，批量逗号查 + 本地缓存价格。
- 欧服挂单也可能稀疏 + 离群噪音 → 估值取多城中位/最低正价，或用 `/history` 近 N 天均价兜底。
...
- `/设置 会员身份组 @身份组`
- `/设置 审批频道 #频道`
- `/设置 运营初始化频道` → 新建或复用 `eu-📡运营中心`，并写入审批、成员变动、击杀、阵亡、战报频道配置
- `/设置 补装初始化频道` → 新建或复用 `eu-🛡️补装中心` 四频道并写入配置
...
- `/设置 成员变动频道 #频道`
- `/设置 大额阈值 <fame>`  → 旧兼容入口；当前只提示固定大额规则，不再修改运行阈值
- `/解绑公会`
...
- 死亡播报：定时轮询全局 `/events` 多页，按 killer/victim 双向筛出本会击杀/阵亡，分别推到当前区服前缀的对应播报频道，未单独配置则推到当前区服前缀的统一播报兜底频道
  - 区分**我方击杀 / 我方阵亡**；击杀/死亡声望大于 100 万，或银币总损失大于 1000 万时按**大额**高亮推送
  - 注意 KOOK 每日发消息上限 1 万，控频 + 去重
- 退会复查：每日比对 `/guilds/{id}/members` → 退会自动撤销身份组，并通知当前区服前缀的成员变动频道
- 价格参考库刷新：每 3 天刷新 T4-T8 主手/双手/副手低价参考
- 战报自动推送：北京时间 14:30 到次日 05:00 每 3 分钟检查候选战役，优先使用官方 `/battles?guildId=`，AlbionBB 仅作补充，按绑定公会、本会最小参战人数和持久去重表过滤后推送到当前区服前缀的专属战报频道；代码路径和离线测试已完成，欧服 KOOK/线上活测待明确实例后执行
...
│   ├── ai/                  # SenseNova/OpenAI 兼容 AI 辅助，只读路由和安全输出层
│   ├── commands/
│   │   ├── admin.py         # 绑定公会 / 设置
│   │   ├── ai.py            # /助手 /战报 /补装解释
...
| **AI 辅助** | `/助手`、`/战报`、`/补装解释`，只读事实包 + 输出安全层 | SenseNova 探针通过；只读边界和危险声明拦截有单测 |
| **版本号控制** | `bot/version.py` 统一版本，`/ping` 带版本 | `tests/test_version.py` 通过 |
| **战报聚合/自动推送** | 生成 ZvZ 聚合报告和 KOOK 卡片，按专属频道自动推送并持久去重 | 已完成离线测试和历史 KOOK 专属频道发送验证；欧服线上服务验证待明确实例后执行 |
| **M7 出勤（快照）** | `/出勤` 最近 N 场参战者聚合 | 出成员出勤次数快照；前端/采集/CTA 拆分见 `docs/superpowers/plans/2026-06-24-m7-attendance-dashboard-roadmap.md` |
...
3. **欧服市场也可能稀疏 + 离群噪音** —— 估值做兜底（多城中位/历史均价），不取单点。
4. **KOOK 每日发消息上限 1 万** —— 死亡播报要控频 + 去重，避免刷爆配额。
5. **Token 安全** —— 走 env，不入库不进 git；`.env` 加 `.gitignore`。
6. **区服别混** —— 战斗 ams / 市场 europe / ZvZ eu / KOOK `eu-` 频道前缀都得是欧服。
...
M0-M6、AI 辅助、战报聚合/自动推送、双区服部署边界等已完成内容，统一归档到：

- `docs/archive/2026-06-24-m0-m6-completed.md`

主计划后续不再把这些内容作为待办重复追踪；只在本文件保留当前架构、命令清单、风险边界和下一阶段入口。需要看最新运行状态、真实验证证据或部署事实时，以 `STATUS.md` 和 `notepad.md` 为准。

## 十二、下一步优先度

后续开发按优先度排序推进，完整任务拆解见 `docs/superpowers/plans/2026-06-24-m7-attendance-dashboard-roadmap.md`：

| 优先级 | 阶段 | 内容 | 说明 |
|---|---|---|---|
| P0 | M7a | `/出勤` 最近 N 场战斗参与快照 | 先给 KOOK 内可用结果，不声明正式考勤 |
| P1 | M7b + M8a | SQLite 快照持久化 + 只读 Web API | 为前端和趋势展示建立同一数据事实 |
| P2 | M8b + M8c | 前端状态页 + 数据采集扩展 | 展示 bot 状态、邀请按钮、榜单、高声望事件、出勤快照 |
| P3 | M9 | 正式 CTA 出勤系统 | 等公会确认活动级考勤规则后再做 |

展示排序同样按优先度收口：`/出勤` 优先展示 Top 参与成员和低参与/未参与摘要；前端榜单默认 Top 20，可配置到 Top 50；高声望事件只展示最近窗口内命中数据。正式 CTA 上线前，所有结果都标注为“战斗参与快照”，不自动用于奖惩或补贴资格判定。
```
