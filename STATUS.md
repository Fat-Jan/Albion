...
- M0-M6 已实现并线上运行。
- 仓库与分支真相：远端仓库为 `https://github.com/Fat-Jan/Albion.git`；`/Users/arm/Desktop/vscode/Albion-ASIA-kook` 固定使用 `deploy/asia` 分支维护亚服，`/Users/arm/Desktop/vscode/Albion-EU-kook` 固定使用 `deploy/eu` 分支维护欧服；`main` 只当共享上游主线，不直接作为某个服务器实例的长期工作分支。
...
- 2026-06-18：追查用户指出的 3 个旧播报频道后确认：`📯丨战报推送` `8139656704033247`、`击杀播报` `5938739897296829`、`死亡播报` `4201481428779754` 此前确实承载过亚服战报/击杀/阵亡推送；但当前亚服测试服已迁移到 `asia-` 前缀专用频道，KOOK 侧这 3 个旧频道已由用户点名授权删除。删除后复查 KOOK guild `4676167053713576` 为 9 个分组、57 个频道，`old_deleted_remaining=none`，`asia_required_missing=none`。SQLite 追查发现本地仍残留 `broadcast_channel_id=5938739897296829`，已先备份数据库到 `data/backups/bot-before-clear-deleted-broadcast-channel-20260618_003804.db`，再清空该兜底字段；复查 `broadcast_channel_id=None`，`kill_broadcast_channel_id=4326560318750543`，`death_broadcast_channel_id=5193310241387334`，`battle_report_channel_id=3891092612097998`，`member_change_channel_id=1203064556541945`，`old_three_hits=none`，`pragma integrity_check=ok`。本地亚服 bot 仍为 PID `77525`，cwd `/Users/arm/Desktop/vscode/Albion-ASIA-kook`。
- 2026-06-18：同步修正亚服测试服频道布局，避免继续使用无前缀补装频道与欧服混淆。使用 `scripts/ensure_region_channels.py --guild-id 4676167053713576 --write-db` 创建并写回 `asia-` 前缀分组/频道，只复用当前区服前缀频道，不复用无前缀或 `eu-` 频道。当前亚服测试服 `4676167053713576` 布局：`asia-📡运营中心` 分组 `3836048705229765`、`asia-🛡️补装中心` 分组 `8151035053984317`、`asia-✅绑定审批` `6280501542155342`、`asia-📢成员变动` `1203064556541945`、`asia-⚔️击杀播报` `4326560318750543`、`asia-💀阵亡播报` `5193310241387334`、`asia-🗺️战报推送` `3891092612097998`、`asia-📥补装申请` `8092855830265715`、`asia-🔍补装审核` `2772001930672157`、`asia-💰补装发放` `9380523626852434`、`asia-📣补装通知` `9980001624642186`。SQLite 复查 `pragma integrity_check=ok`；与欧服 `4676167053713576` 的运营/补装 9 个频道字段交叉比对 `shared_count=0`。本地亚服 bot 已重启为 PID `77525`，日志 `logs/bot-asia-region-20260617.log` 显示 `bot_id=49025 token_fp=45a5a99e7b1b token_source=env_file` 和 `khl.receiver: [ init ] launched`；本机只剩欧服 PID `77524` 与亚服 PID `77525` 两个 `python -m bot.main` 进程。追加验证：亚服 `scripts/check.sh` 通过（164 个单元测试 + `compileall bot scripts tests`），欧服 `scripts/check.sh` 通过（165 个单元测试 + `compileall bot scripts tests`）；`lsof` 确认 PID `77525` cwd 为 `/Users/arm/Desktop/vscode/Albion-ASIA-kook`、PID `77524` cwd 为 `/Users/arm/Desktop/vscode/Albion-EU-kook`。
