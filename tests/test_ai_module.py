    ...
    async def test_router_rejects_mutating_intent(self):
        ...
        for question in dangerous_questions:
            with self.subTest(question=question):
                text = await router.answer("guild", "admin", question)
...
class AIContextTest(unittest.TestCase):
    ...
    def test_configured_display_name_mention_triggers_and_is_removed(self):
        msg = SimpleNamespace(content="@Jianguomao 帮我调用战报", mention=[])

        self.assertTrue(
            ai_commands._message_mentions_bot(msg, "49050", names=("Jianguomao",))
        )
        self.assertEqual(
            ai_commands._strip_bot_mention(
                msg.content, "49050", names=("Jianguomao",)
            ),
            "帮我调用战报",
        )
