...
class RegionScopeTest(unittest.IsolatedAsyncioTestCase):
    def test_asia_default_prefixes_regear_channel_names(self):
        self.assertEqual(region_scope.region_code(), "asia")
        self.assertEqual(region_scope.scoped_name("📥补装申请"), "asia-📥补装申请")
    ...
    def test_process_message_only_in_region_channel(self):
        msg = SimpleNamespace(
            ctx=SimpleNamespace(channel=SimpleNamespace(name="eu-📥补装申请"))
        )
    ...
    def test_bootstrap_does_not_allow_other_region_channels(self):
        msg = SimpleNamespace(ctx=SimpleNamespace(channel=SimpleNamespace(name="eu-📥补装申请")))
    ...
    def test_region_code_can_be_overridden_for_eu(self):
        old = os.environ.get("KOOK_REGION_CODE")
        os.environ["KOOK_REGION_CODE"] = "eu"
        try:
        ...
        guild = SimpleNamespace(
            fetch_channel_list=lambda: _async_value(
                [
                    ...
                    SimpleNamespace(id="222", name="eu-📥补装申请"),
                    SimpleNamespace(id="333", name="asia-📥补装申请"),
                ]
            )
        )

        self.assertEqual(await admin._resolve_channel(guild, "📥补装申请"), "333")
        self.assertIsNone(await admin._resolve_channel(guild, "eu-📥补装申请"))
    ...
    async def test_initialize_channels_creates_scoped_operations_and_regear_groups(self):
        ...
        self.assertEqual(ops_category.name, "asia-📡运营中心")
        self.assertEqual(regear_category.name, "asia-🛡️补装中心")
        self.assertEqual(
            {field: channel.name for field, channel in ops_channels.items()},
            {
                "approval_channel_id": "asia-✅绑定审批",
                "member_change_channel_id": "asia-📢成员变动",
                "kill_broadcast_channel_id": "asia-⚔️击杀播报",
                "death_broadcast_channel_id": "asia-💀阵亡播报",
                "battle_report_channel_id": "asia-🗺️战报推送",
            },
        )
        self.assertEqual(
            {field: channel.name for field, channel in regear_channels.items()},
            {
                "regear_apply_channel_id": "asia-📥补装申请",
                "regear_review_channel_id": "asia-🔍补装审核",
                "regear_payout_channel_id": "asia-💰补装发放",
                "regear_notify_channel_id": "asia-📣补装通知",
            },
        )
    ...
    async def test_admin_initializers_reuse_categories_from_category_list(self):
        guild = FakeSplitListGuild(
            categories=[
                FakeChannel("ops-category", "asia-📡运营中心"),
                FakeChannel("regear-category", "asia-🛡️补装中心"),
            ],
            channels=[],
        )
    ...
    def test_ensure_layout_ignores_plain_and_other_region_channels(self):
        ...
        self.assertEqual(layout["regear_apply_channel_id"].channel.id, "asia")
    ...
    def test_ensure_layout_marks_missing_region_channel_for_creation(self):
        ...
        self.assertEqual(layout["regear_apply_channel_id"].name, "asia-📥补装申请")
