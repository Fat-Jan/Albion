...
async def _resolve_channel(guild, value: str, *, region: str = "eu"):
    """频道：优先 #提及/数字 id，回退按名字匹配。返回 id 字符串或 None。"""
    if region_scope.has_known_region_prefix(value) and not region_scope.channel_name_matches_region(value, region=region):
        return None
    ...
    if cid:
        for c in channels:
            if str(getattr(c, "id", "")) == str(cid) and region_scope.is_reusable_region_channel(c, region=region):
                return cid
        return None
    wanted = {
        str(value or "").strip(),
        region_scope.scoped_name(value, region=region),
        region_scope.strip_known_prefix(value),
    }
    for c in channels:
        name = getattr(c, "name", None)
        if name in wanted and region_scope.is_reusable_region_channel(c, region=region):
            return c.id
    return None
...
async def _create_regear_center(
    bot: Bot, guild, binding: dict, *, region: str = "eu"
) -> tuple[object, dict[str, object], list[str]]:
    category, channels = await _create_regear_center_channels(guild, region=region)
    warnings = await _apply_regear_center_permissions(bot, guild, channels, binding)
    return category, channels, warnings


async def _find_region_category(
    guild, base_name: str, fallback_channels: list[object], *, region: str = "eu"
) -> object | None:
    scoped_category_name = region_scope.scoped_name(base_name, region=region)
    categories = []
    ...
    for channel in list(categories or []) + list(fallback_channels or []):
        if (
            ...
            and region_scope.is_reusable_region_channel(channel, region=region)
        ):
            return channel
    return None


async def _create_regear_center_channels(guild, *, region: str = "eu") -> tuple[object, dict[str, object]]:
    channels_existing = await guild.fetch_channel_list()
    category = await _find_region_category(
        guild, REGEAR_CENTER_CATEGORY_NAME, channels_existing, region=region
    )
    if category is None:
        category = await guild.create_channel_category(
            region_scope.scoped_name(REGEAR_CENTER_CATEGORY_NAME, region=region)
        )
    channels = {}
    for field, name in REGEAR_CENTER_CHANNELS.items():
        scoped = region_scope.scoped_name(name, region=region)
        existing = next(
            (
                channel
...
async def _create_operations_center(guild, *, region: str = "eu") -> tuple[object, dict[str, object]]:
    channels_existing = await guild.fetch_channel_list()
    category = await _find_region_category(
        guild, OPERATIONS_CENTER_CATEGORY_NAME, channels_existing, region=region
    )
    if category is None:
        category = await guild.create_channel_category(
            region_scope.scoped_name(OPERATIONS_CENTER_CATEGORY_NAME, region=region)
        )
...
async def _ensure_region_scoped_channel_layout(bot: Bot, guild, *, region: str = "eu") -> None:
    await _create_operations_center(guild, region=region)
    await _create_regear_center_channels(guild, region=region)
...
def register(bot: Bot, gi: GameInfo, *, region: str = "eu") -> None:
    ...
    async def on_self_joined_guild(b: Bot, event):
        ...
        try:
            ...
            await _ensure_region_scoped_channel_layout(b, guild, region=region)
        except Exception as exc:
            log.warning("bot 入服后初始化区服频道失败 guild=%s: %s", guild_id, exc)
            return
        log.info("bot 入服后已创建/复用区服频道布局 guild=%s region=%s", guild_id, region)

    @bot.command(name="绑定公会")
    async def bind_guild_cmd(msg: Message, *args):
        if not region_scope.should_process_message(msg, allow_bootstrap=True, region=region):
            return
    ...
    async def unbind_guild_cmd(msg: Message, *args):
        ...
        if not await perms.is_guild_admin(msg.ctx.guild, msg.author):
            ...
            return
        ok = repo.unbind_guild(msg.ctx.guild.id, region)
    ...
    async def setting_cmd(msg: Message, *args):
        ...
        if not region_scope.should_process_message(
            msg, allow_bootstrap=key in ("运营初始化频道", "补装初始化频道"), region=region
        ):
            return
        if not await perms.is_guild_admin(msg.ctx.guild, msg.author):
        ...
        if not repo.get_guild_binding(kgid, region):
        ...
        if key == "会员身份组":
            ...
            if not rid:
                ...
                return
            repo.set_setting(kgid, region, "member_role_id", rid)
            await msg.reply(f"✅ 会员身份组已设为 (rol){rid}(rol)")

        elif key == "审批频道":
            cid = await _resolve_channel(msg.ctx.guild, value, region=region)
            if not cid:
                await msg.reply(f"找不到频道「{value}」。可 #频道、给 ID，或确认名字完全一致。")
                return
            repo.set_setting(kgid, region, "approval_channel_id", cid)
            ...
            if not cid:
                await msg.reply(f"找不到频道「{value}」。可 #频道、给 ID，或确认名字完全一致。")
                return
            repo.set_setting(kgid, region, "broadcast_channel_id", cid)
            ...
            if not cid:
                await msg.reply(f"找不到频道「{value}」。可 #频道、给 ID，或确认名字完全一致。")
                return
            repo.set_setting(kgid, region, "kill_broadcast_channel_id", cid)
            ...
            if not cid:
                await msg.reply(f"找不到频道「{value}」。可 #频道、给 ID，或确认名字完全一致。")
                return
            repo.set_setting(kgid, region, "death_broadcast_channel_id", cid)
            ...
            if not cid:
                await msg.reply(f"找不到频道「{value}」。可 #频道、给 ID，或确认名字完全一致。")
                return
            repo.set_setting(kgid, region, "battle_report_channel_id", cid)
            ...
            repo.set_setting(kgid, region, "battle_report_min_guild_players", count)
            ...
            if not cid:
                await msg.reply(f"找不到频道「{value}」。可 #频道、给 ID，或确认名字完全一致。")
                return
            repo.set_setting(kgid, region, "member_change_channel_id", cid)
            await msg.reply(f"✅ 成员变动频道已设为 (chn){cid}(chn)")

        elif key == "运营初始化频道":
            try:
                category, channels = await _create_operations_center(msg.ctx.guild, region=region)
            except Exception as exc:
                log.warning("初始化运营中心失败: %s", exc)
                await msg.reply("⚠️ 创建运营中心失败，请检查 bot 是否有管理频道/管理权限。")
                return
            for field, channel in channels.items():
                repo.set_setting(kgid, region, field, str(channel.id))
            lines = [
                f"✅ 已创建/复用 `{region_scope.scoped_name(OPERATIONS_CENTER_CATEGORY_NAME, region=region)}`，并写入运营频道配置。",
                f"· 分组：`{getattr(category, 'id', '-')}`",
            ]
            for field, name in OPERATIONS_CENTER_CHANNELS.items():
                lines.append(f"· {region_scope.scoped_name(name, region=region)}：(chn){channels[field].id}(chn)")
            await msg.reply("\n".join(lines))

        elif key == "补装初始化频道":
            binding = repo.get_guild_binding(kgid, region) or {}
            try:
                category, channels, warnings = await _create_regear_center(
                    bot, msg.ctx.guild, binding, region=region
                )
            except Exception as exc:
            ...
            for field, channel in channels.items():
                repo.set_setting(kgid, region, field, str(channel.id))
            lines = [
                f"✅ 已创建/复用 `{region_scope.scoped_name(REGEAR_CENTER_CATEGORY_NAME, region=region)}`，并写入补装频道配置。",
                f"· 分组：`{getattr(category, 'id', '-')}`",
            ]
            for field, name in REGEAR_CENTER_CHANNELS.items():
                lines.append(f"· {region_scope.scoped_name(name, region=region)}：(chn){channels[field].id}(chn)")
            ...
            repo.set_setting(kgid, region, field, cid)
            ...
            if not cid:
                await msg.reply(f"找不到频道「{value}」。可 #频道、给 ID，或确认名字完全一致。")
                return
            repo.set_setting(kgid, region, "regear_channel_id", cid)
            ...
            if not rids:
                ...
                return
            repo.set_setting(kgid, region, "regear_reviewer_role_ids", ",".join(rids))
            ...
            if not rids:
                await msg.reply(f"找不到身份组「{value}」。可 @身份组，或确认名字。")
                return
            repo.set_setting(kgid, region, "trusted_role_ids", ",".join(rids))
    ...
    async def on_btn_click(b: Bot, event):
        ...
        try:
            ...
            log.warning("拉取频道失败: %s", exc)
            return
        if not region_scope.channel_allowed(channel, allow_bootstrap=True, region=region):
            return
        ...
        repo.bind_guild(kgid, region, gid, gname, user_id)
        await channel.send(
            f"✅ 已将本服务器绑定到公会「{gname}」。\n"
