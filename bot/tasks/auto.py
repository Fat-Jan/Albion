...
- 死亡播报：每 2 分钟轮询全局 /events（多页），筛出本会击杀/阵亡推到播报频道，
...
def register(bot: Bot, gi: GameInfo, mk: Market) -> None:
    @bot.task.add_interval(minutes=BROADCAST_INTERVAL_MIN)
    async def death_broadcast():
        global _primed
        targets = {
            gb["albion_guild_id"]: gb
            for gb in repo.all_guild_bindings()
            if gb.get("broadcast_channel_id")
        }
        if not targets:
            return
    ...
    async def member_review():
        for gb in repo.all_guild_bindings():
            ...
            notify_id = gb.get("broadcast_channel_id") or gb.get("approval_channel_id")
