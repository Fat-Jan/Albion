...
审核频道、发放频道和补装通知卡会显示申请号、当前状态、死亡摘要、装备文字明细、补装金额和带事件号的官方击杀板链接；待审批卡也会显示申请号和待审批状态。通过、拒绝、发放都会在补装通知频道 @ 申请人，拒绝会记录原因，发放会记录处理时间和发放方式。数据库中的处理/审核/发放时间在 KOOK 卡片里统一显示为北京时间并标注。按钮处理后会尽量把原审核卡原地更新为已通过、已拒绝或已发放；如果 KOOK 消息更新失败，仍会发送新的状态通知。
