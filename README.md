...
- 本 README 对应亚服实例分支 `deploy/asia`，本地目录固定为 `/Users/arm/Desktop/vscode/Albion-ASIA-kook`。
- 同一远端仓库还维护欧服实例分支 `deploy/eu`，本地目录固定为 `/Users/arm/Desktop/vscode/Albion-EU-kook`。
- `main` 只作为共享上游主线，不直接作为亚服或欧服服务器实例的长期工作分支。
- 共享功能修复先做成独立提交，分别在 `deploy/asia` 与 `deploy/eu` 之间 `cherry-pick`；不要整分支互 merge，避免区服默认值、`.env.example`、公会名、运行证据和部署记录串区。
- 亚服分支必须保持 `KOOK_REGION_CODE=asia`、亚服数据源和 `asia-` 频道前缀一致；切区时需要同时切频道前缀、官方 gameinfo、AODP、AlbionBB API、AlbionBB 网页和官方击杀板 server。
...
- 区服作用域由 `KOOK_REGION_CODE=asia` 和同区数据源共同决定；本实例只响应 `asia-` 前缀频道里的 `/指令` 和 `@机器人` 自然语言，其他前缀或无前缀频道会静默。
- 一个 KOOK 服务器绑定一个 Albion 公会；公会、成员、频道、身份组、补装和战报去重状态保存在本地 SQLite。
- 频道配置保存的是 KOOK 频道 ID，但运行时要求频道名保留当前区服前缀。删除频道、重建频道、去掉 `asia-` 前缀或迁移到新服务器后，需要重新执行对应 `/设置 ...频道 #频道` 或初始化命令。
...
- ZvZ 战报聚合、专属频道配置、最小本会参战人数阈值、持久去重和自动定时推送代码路径已有离线测试覆盖；自动推送最低 20 名本会参战者才会发送，开启 AI 后会附带 AI 摘要。
...
- `/设置 运营初始化频道`：自动新建或复用 `asia-📡运营中心` 分组和审批、成员变动、击杀、阵亡、战报频道，并写入配置。
- `/设置 补装初始化频道`：自动新建或复用 `asia-🛡️补装中心` 分组和四个补装频道，并写入配置。
...
- `/设置 战报本会最小人数 <人数>`：配置自动战报推送的本会参战人数阈值，默认 20 人，低于 20 会按 20 生效。
...
频道设置写入的是 KOOK 频道 ID；频道名必须保留 `asia-` 前缀。只改前缀后的说明文字通常不需要重新配置，删除频道、重建频道或去掉 `asia-` 前缀后才需要重新 `/设置`。
...
- `/设置 补装初始化频道`：推荐初始化方式，新建或复用 `asia-🛡️补装中心`、`asia-📥补装申请`、`asia-🔍补装审核`、`asia-💰补装发放`、`asia-📣补装通知`。
...
```text
asia-🛡️补装中心
├─ asia-📥补装申请      成员可见，可发 /补装 /补装状态
├─ asia-🔍补装审核      仅补装组/管理可见
├─ asia-💰补装发放      仅补装组/发放组/管理可见
└─ asia-📣补装通知      成员可见，只发完成通知
```

### 自动任务

- 死亡播报：普通时段每 4 分钟轮询全局事件，20:00-00:30 每 90 秒，筛选本会击杀和阵亡；可分别推送到击杀播报和阵亡播报频道。大额高亮固定为击杀/死亡声望大于 100 万，或银币总损失大于 1000 万。
- 退会复查：每天 04:00 比对 Albion 公会成员，退会则撤 KOOK 会员组、删除绑定，并通知成员变动频道。
- 价格参考库刷新：每 3 天刷新 T4-T8 主手、双手、副手低价参考。
- ZvZ 战报推送：默认北京时间 14:30 到次日 05:00 每 15 分钟检查一次，可通过 `DISPLAY_TZ` 和 `BATTLE_REPORT_WINDOW_*` 调整；命中绑定公会且至少 20 名本会参战者后推送专属战报卡；开启 AI 时，卡片会附带只读 AI 摘要。
...
`KOOK_TOKEN` 是必填密钥。`KOOK_REGION_CODE` 决定频道前缀和消息响应作用域，亚服使用 `asia`。启用 AI 时使用 `AI_API_KEY` 或 `LONGCAT_API_KEY`，前者优先。`KOOK_BOT_MENTION_ALIASES` 用来配置 `@机器人` 自然语言入口额外可识别的显示名别名，默认留空；多个别名用英文逗号分隔。密钥不要提交到 Git。

默认配置面向亚服。切区时需同时切 `KOOK_REGION_CODE`、官方 gameinfo、AODP、AlbionBB API、AlbionBB 网页和官方击杀板 server，避免频道、市场、战报和链接混区。例如欧服：
...
当前新加坡服务器按双实例隔离：亚服使用 `/opt/albion-kook-asia` 与 `albion-kook-asia.service`，欧服使用 `/opt/albion-kook-eu` 与 `albion-kook-eu.service`。旧 `/opt/albion-kook` 单服务已归档移除，不要恢复为运行入口。

同一个 `KOOK_TOKEN` 不能被两个正在运行的 bot 进程同时使用，否则会抢 KOOK WebSocket 连接并可能重复处理消息。调试亚服线上同一个 bot token 前，先停止亚服服务：

```bash
systemctl stop albion-kook-asia.service
.venv/bin/python -m bot.main
systemctl start albion-kook-asia.service
```
...
当前亚服线上实例的 systemd 与日志命令：

```bash
systemctl status albion-kook-asia.service --no-pager --lines=80
systemctl restart albion-kook-asia.service
journalctl -u albion-kook-asia.service -n 100 --no-pager
tail -80 /var/log/albion-kook-asia/bot.log
```
