...
- 测试用 KOOK 服务器可换（当前 id `4676167053713576`，bot 身份 Jianguomao#7691 只是当前测试身份）；绑定一律按运行时 guild_id 走，代码不写死服务器，机器人显示名别名也只通过 `KOOK_BOT_MENTION_ALIASES` 显式配置。
...
- 本地双版本维护约定：亚服目录 `/Users/arm/Desktop/vscode/Albion-ASIA-kook` 固定使用 `deploy/asia` 分支；欧服目录 `/Users/arm/Desktop/vscode/Albion-EU-kook` 固定使用 `deploy/eu` 分支；`main` 只当共享上游主线，不直接作为某个服务器实例长期开发分支。双目录 VS Code 入口在 `/Users/arm/Desktop/vscode/Albion-ASIA-EU-kook.code-workspace`。
...
4. **M6 播报/成员变动**：`/设置 播报频道 <频道>`、`/设置 成员变动频道 <频道>` → 普通时段等 4 分钟、20:00-00:30 等 90 秒，有 Mika 击杀/阵亡会推卡片；击杀/死亡声望大于 100 万，或银币总损失大于 1000 万时大额高亮。退会复查是每日 4 点 cron，通知发成员变动频道。
