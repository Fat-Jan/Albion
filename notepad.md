...
- OpenDeploy 当前已完成合并双区部署冒烟并更新到本轮修复包：复用 project `32e65f76-29f3-401c-8c59-b689761f768d`，service `cd6868e9-cb87-47dd-ba88-0c56a995f59c`，live URL `https://cd6868e9.opendeploy.site`，current deployment `0c6446e7-a189-4f00-8041-31ef9835d257` / version `1.0.5`，2026-07-02 22:00 CST 完成且 `status=success`、`is_current=true`。服务 `status=running`，Dockerfile builder，`start_command=python -m bot.main`，`port=8080` locked，`/app/data` 1Gi volume `albion-eu-data` active；runtime variable key 含 `KOOK_TOKEN_EU`、`KOOK_TOKEN_ASIA`、EU/ASIA 数据源和 SenseNova AI key，已执行 OpenDeploy env patch 设置 `AI_ENABLED=true`，未使用 `--show-secrets`，未读取密钥原文。本轮公网复核：`/healthz` HTTP 200 `ok`；`/` 已是默认中文运营台并含 `lang-switcher` / `data-i18n`；`/app.js` 已含轻量 I18N、`localStorage` 语言偏好、`setLang`、`t(key)`；`/api/attendance/recent?limit=2` 返回 EU/ASIA 各 1 条；`/api/status` 在首个 5 分钟 interval 后返回 `last_task_run=2026-07-02T14:05:46Z`、collector_summary `ok` total 4，EU/ASIA 两区均 `ok` total 2。OpenDeploy deployment 日志显示 K8s `Deployment successful`；CLI `logs diagnose` 仍可能给出泛化 `port_or_listener` 分类，但公网 HTTP 和 status API 正常，先不作为失败判断。KOOK 真人 `/ping` 与 `@bot` AI 入口尚未活测，旧 ASIA project 尚未停机。2026-07-02 早上的 1.0.3 / `d3a2f7d3-b6b5-4ee2-9291-d7eb84f380ec` 记录已被本条 supersede。
