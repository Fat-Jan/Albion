...
- 当前进度：**M0-M6 已实现，欧服已在新加坡服务器按双区服隔离方式上线；M7 P0/P1/P2 已离线完成并做过数据/API/前端硬化**。欧服本地代码、配置默认值和文档默认标识已适配；旧 `/opt/albion-kook` 单服务已归档并移除，当前欧服线上实例是 `/opt/albion-kook-eu` + `albion-kook-eu.service`。当前项目版本 `1.0`（`bot/version.py`，`/ping` 返回 `pong v1.0`）。AI 辅助已扩展为高频只读露出：`/助手`、`/战报 [日期]`、`/补装解释`、补装审核卡「AI 审核提示」、自动 ZvZ 战报卡「AI 摘要」；`/战报 6-15` 等日期参数按北京时间目标日 14:30 到次日 05:00 过滤。AI 仍不得审批、发组、撤组、改金额或标记发放。M7 已完成 `/出勤 [5-50]` 最近 N 场战斗参与快照、SQLite 出勤缓存、近期战斗/成员/高声望事件/榜单/金价采集器、只读 API 和静态前端面板；本轮已补 dashboard 查询索引、榜单/金价快照保留策略、P2 API `?limit=`、`/api/status` collector health 汇总、深色运维 dashboard、顶部 Ops 摘要栏、Health 速读卡、前端加载/错误/disabled invite 状态、紧凑 empty 状态和移动端稳定性，并继续修正同场数成员按最近参战排序、dashboard 出勤参与场数字段、`/出勤` 无成员分母缓存回退实时拉取并写回共享 SQLite 缓存、`battle_snapshot` 足够但缺 `battle_participant` 明细时不命中缓存、dashboard 最近战斗展示，以及 `battle_snapshot` 按 `(kook_guild_id, battle_id)` 隔离以支持旧 KOOK guild 与 fumass 共用同一 Albion battle；P3 正式 CTA 出勤仍待公会规则确认。
- OpenDeploy 当前已完成合并双区部署冒烟：复用 project `32e65f76-29f3-401c-8c59-b689761f768d`，service `cd6868e9-cb87-47dd-ba88-0c56a995f59c`，live URL `https://cd6868e9.opendeploy.site`，current deployment `d3a2f7d3-b6b5-4ee2-9291-d7eb84f380ec` / version `1.0.3`，2026-07-02 06:45 CST 完成且 `status=success`、`is_current=true`。服务 `status=running`，Dockerfile builder，`start_command=python -m bot.main`，`port=8080` locked，`/app/data` 1Gi volume `albion-eu-data` active；runtime variable key 已切到双区命名（含 `KOOK_TOKEN_EU`、`KOOK_TOKEN_ASIA`、EU/ASIA 数据源和 SenseNova AI key），未使用 `--show-secrets`，未读取密钥原文。2026-07-02 07:08 CST 复核：公网探针（curl/浏览器 UA）确认 `/healthz` HTTP 200 `ok`、`/api/status` HTTP 200，`last_heartbeat=2026-07-01T23:07:57Z`、`last_task_run=2026-07-01T23:05:02Z`、collector_summary `ok` total 6，EU/ASIA 两区 collector_summary 均为 `ok` total 3；`/api/attendance/recent?limit=2` 返回 EU/ASIA 各 1 条；`/api/market/gold?limit=2` 返回 1 组快照（`captured_at=2026-07-01T23:00:02Z`）；`/api/invites` 仍未配置邀请链接。2026-07-02 07:21 CST 再复核：deployment 仍 `success/current`、service `running`，`/healthz` 返回 `ok`，`/api/status` 返回 `last_heartbeat=2026-07-01T23:21:43Z`、`last_task_run=2026-07-01T23:20:02Z`、collector_summary `ok` total 6，EU/ASIA 两区均 `ok` total 3。runtime 日志确认 EU bot `49050` 与 ASIA bot `49025` 两个 KOOK WebSocket receiver 均启动，seeded runtime guild config 分别为 `Top Squad` / `Mika`，`gameinfo-ams` 和 `gameinfo-sgp` 请求成功，错误扫描 `matches=0`。OpenDeploy 内置 `project-health` 仍显示 `health_status=unknown` / `ready_replicas=0`，但公网 HTTP、service 状态和 runtime 日志均正常；KOOK 真人 `/ping` 尚未活测，旧 ASIA project 尚未停机。2026-06-24 的 1.0.2 / `1f73fc6f-6f4b-4c90-8dc3-548c252abbc7` 记录已被本条 supersede。
...
| M7 出勤快照 + 前端面板 | ✅ P0/P1/P2 离线完成并已硬化：`/出勤` 快照、SQLite 缓存、采集器、只读 API、静态 dashboard、查询索引、快照保留策略、API 限量和前端加载/错误状态；真实 KOOK 活测/部署待后续 |
...
旧 Mika 活测遗留：测试用户 `1380312587` 的 KOOK 昵称恢复为 `BEISHENGS` 曾被限流阻断，后续如仍使用同一测试用户再重试。当前欧服主线继续按 `Top Squad` 本地绑定复测 AI 只读查询；M7 P0/P1/P2 已离线完成，下一步可部署/访问只读 dashboard 并复测真实 KOOK 交互，正式 CTA 出勤继续等公会规则确认后进入 P3。
