...
2. **换 AI**:LongCat 已停止免费,换商汤 `token.sensenova.cn/v1`,key `sk-OTlM...`
...
```bash
# 1. 列模型
curl -sS -H "Authorization: Bearer sk-OTlM..." https://token.sensenova.cn/v1/models
# 2. 探针 chat completions
curl -sS -X POST https://token.sensenova.cn/v1/chat/completions \
  -H "Authorization: Bearer sk-OTlM..." \
